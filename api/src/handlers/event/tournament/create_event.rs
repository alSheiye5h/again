...
use crate::models::Event_struct::{CreateEventPayload, Event};
use serde_json::json;
use sqlx::PgPool;
...
pub async fn create_event(
    ...
    let event_result = sqlx::query_as::<_, Event>(
        r#"
        INSERT INTO tournament_event (club_host, community_host, organizer, has_discussion)
        VALUES ($1, $2, $3, $4)
    ...
    if event.has_discussion {
        ...
        match discussion_id_result {
            Ok(discussion_id) => {
                if let Err(e) = sqlx::query("INSERT INTO tournament_event_discussion (event_id, discussion_id) VALUES ($1, $2)")
                    .bind(event.id)
                    .bind(discussion_id)
                    .execute(&mut *tx)
                    .await {
