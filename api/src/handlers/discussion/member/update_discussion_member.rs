...
/// Handler to update a member's role in a discussion.
pub async fn update_discussion_member_role(
    ...
    match result {
        Ok(res) if res.rows_affected() > 0 => {
            HttpResponse::Ok().json(json!({"status": "success", "message": "Member role updated successfully."}))
        }
